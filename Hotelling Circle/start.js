...
$(function() {
  ...
  $("#actionCircle").on("click", function(e){
    ...
    if(game_type == "simultaneous" || game_type == "stage"){
      ...
      if (x_rate === 0 && y_rate === 0) {
        ...
        r.send("update_circle", { x_pos:myX , y_pos:myY, id:id });
      } else if (x_rate === 0) {
        r.send("update_loc", { new_loc:new_loc , id:id, iterx:iterx });
        r.send("update_circle_x", { x_pos:myX , id:id });
      } else if(y_rate === 0) {
        r.send("update_circle_y", { y_pos:myY , id:id });
        r.send("update_pos", { new_pos:new_pos , id:id, itery:itery });
      }
  ...
  //plot 1 on hover event handler for drawing crosshairs
  $("#actionCircle").on("hover", function (e){
    var parentOffset = $(this).parent().offset(); 
    ...
    var distance;
    distance = Math.sqrt((relX-225)*(relX-225) + (relY-225)*(relY-225));
    if (distance > 200) console.log("OUTSIDE OF CIRCLE");
