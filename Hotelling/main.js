...
function update_plot(){
  ...
  plot = $.plot("#placeholder", [
    ...
    { data: tmp_a0, hoverable: false, color: player_color(network.players[0].id), lines:{ show: true, fill: false }},
...
function quad_mirror(index) {
  ...
  var location = player.loc;
  var neg_location = location * -1;
 

  for (var x = location; x <= location + 0.5; x = x + 0.01) {

    var loc_x = x;
    if (x > 1) loc_x = x - 1;

    var y = price + Math.pow(Math.abs(neg_location + x), 2);
      ...
      if(i != j){
        if (linear) {
          ...
          if (! ((0 < intersection) && (intersection < 1))) {
            console.log("entering for player with price: " + p1.price);
...
function find_intersect_pts(){
  ...
  if (linear) {
    ...
    for(i = 0; i < tmp.length - 1; ++i){
      ...
      //console.log("top/denom=" + top/denom);
      var intersection = top/denom;
      //console.log("Found a market bound at " + intersection);
...
function get_players(){
  ...
  if(id == keeper) {
    ...
    r.send("data_log", { value:value , curr_subperiods:curr_subperiods, silo_num:silo_num });
  }
...
$(function() {
  ...
  */
  ...
  r.recv("config", function(msg){
    ...
    r.send("setup", { chosen:chosen, keeper:keeper });
  });
  ...
  r.recv("setup", function(msg) {
    ...
    if(id == keeper){
      var tmp = network.players;
      r.send("sync_net", { tmp:tmp });
      r.send("sync_time", { time:time });
    }
    ...
    options = { 
                  ...
                  yaxis: { min: 0, max:1, position: "left", ticks:1, tickDecimals:2, tickColor:'#858585'},
